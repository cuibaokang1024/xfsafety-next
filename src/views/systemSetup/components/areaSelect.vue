      ...
      <el-container>
        ...
        <el-aside>
          <div class="selected-list">
            <ul>
              <li v-for="point in selectedList" :key="point.id" @dblclick="delSelected(point)">
                {{ point.num }}
              </li>
            </ul>
          </div>
...
export default {
  ...
  methods: {
    ...
    _getLabel() {
      getLabel().then((res) => {
        if (res.data.length > 0) {
          this.searchFormOption.config = this.searchFormOption.config.map((item) => {
            if (item.name === 'typeId') {
    ...
    _getPartsAddrList(node, resolve) {
      getPartsAddrList({ partsId: node.data.id }).then((res) => {
        if (res.data) {
          resolve(res.data)
        }
      }).catch((error) => { console.log(error) })
