<template>
  <div style="height: 100%">
    <transition name="fade-transform" mode="out-in">
      <div class="section-main fireOffice">
        <div class="left-container">
          ...
          />
          <TreeNode v-else height="722px" :tree-data="treeData" @click="handleNodeClick" @handleNodeClick="handleNodeClick" />
        </div>
    ...
    <!-- 弹出框 -->
    <transition name="fade-transform" mode="out-in">
      <UserManage v-if="isShowUserManage" :user-type="2" :office-id="viewUserOfficeId" :office-name="viewUserOfficeName" @hideUserManage="hideUserManage" />
    </transition>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    createData(data) {
      ...
      saveFireOffice(formData).then(res => {
        if (res.msg === 'ok') {
          this._handleRightTable()
    ...
    updateData(data) {
      ...
      updateFireOffice(formData).then(res => {
        if (res.msg === 'ok') {
          this._handleRightTable()
          this.handleRealUpdateData(data.parentId)
          this.$message({
