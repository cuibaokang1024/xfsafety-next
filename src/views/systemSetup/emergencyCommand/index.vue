    ...
    <div class="search">
      ...
      <transition name="fade">
        <div v-show="isShowMatchingList" ref="matchingOfficeList" class="matchingOfficeList">
          ...
          <ul>
            <li v-for="office in matchingOfficeList" :key="office.id" :class="{active:selectedOfficeId === office.id }" @click="showOfficeInfo(office.id)">{{ office.name }}</li>
          </ul>
        ...
        <transition name="slide">
          <div v-show="isShowOfficeInfo&&isShowBuildingPlan" class="buildingPlan box">
            ...
            <div class="image-wrapper">
              <template v-for="(img,index) in buildingPlanUrls">
                <el-image :key="img.url" :src="img.url" lazy />
                <div :key="index" class="name">{{ img.name }}</div>
              </template>
            </div>
    ...
    >
      <template v-slot:fireOfficer>
        <el-input v-model="fireOfficerNames" placeholder="请选择人员" class="input-with-select">
          <el-button slot="append" icon="el-icon-search" @click="showFireOfficerSelect" />
        </el-input>
      </template>
    </BaseForm>
    <el-dialog
...
export default {
  ...
  methods: {
    ...
    createData (data) {
      ...
      const longitude = this.queryMarker.getPosition().lng
      const latitude = this.queryMarker.getPosition().lat
      const params = {
        ids,
        longitude,
        latitude,
    ...
    // 显示单位信息
    async showOfficeInfo () {
      this.isShowMatchingList = false
      const officeInfo = this.officeInfo
