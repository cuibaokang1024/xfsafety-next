  ...
  <div class="main">
    <div class="container">
      <div class="left-container">
        ...
        <div class="box bot">
          <ul class="list">
            <li @click="filterOffice(query={isAlert:'13',sensorCat: 4})">
              <div class="icon co" />
              <div class="desc">
                <span class="text">一氧化碳</span>
                <span class="num-wrapper">
                  <span class="num">
                    <number-roll
                      :width="'10px'"
                      :font-size="'18px'"
                      :number="monitorData.conum"
                      :unit="'套'"
                    />
                  </span>
                </span>
            ...
            <li @click="filterOffice(query={isAlert:'12'})">
              <div class="icon manipulator" />
              <div class="desc">
                <span class="text">机械手</span>
                <span class="num-wrapper">
                  <span class="num">
                    <number-roll
                      :width="'10px'"
                      :font-size="'18px'"
                      :number="monitorData.smknum"
                      :unit="'套'"
                    />
                  </span>
                </span>
            ...
            <li @click="filterOffice(query={isAlert:'11'})">
              <div class="icon leakage" />
              <div class="desc">
                <span class="text">燃气泄露</span>
                <span class="num-wrapper">
                  <span class="num">
                    <number-roll
                      :width="'10px'"
                      :font-size="'18px'"
                      :number="monitorData.batnum"
                      :unit="'套'"
                    />
                  </span>
                </span>
      ...
      <div class="right-container">
        ...
        <div v-show="visibility==='alarm'" class="tool">
          ...
          <span class="clear" @click="clearAlarmList">清空</span>
        </div>
    ...
    <alarm-handle ref="alarmHandle" :dev-category="3" :dev-id="viewDevid" :office-id="viewOfficeId" :is-reset-flag="isDetailResetFlag" />
    <history-alarm ref="historyAlarm" :is-reset-flag="isHistoryResetFlag" :dev-category="3" />
    <el-dialog
      v-if="dialogVisible"
    ...
    >
      ...
      <div class="dialog-content">
        <unit-dev-detail :office-id="viewDevOfficeId" :category="3" :category-name="'gas'" />
      </div>
...
import { getMonitorData, getOfficeAlert, getOfficeList } from '@/api/gas'
...
export default {
  ...
  data() {
    return {
      ...
      activeName: 'electric',
      latestData: {}, // mqtt最新数据
      visibility: 'alarm', // 显示社会单位列表（office） | 报警列表（alarm）
      monitorData: {
        title: '可燃气体监测',
        chartId: 'gasPieChart'
      },
  ...
  methods: {
    // 获取左侧监测数据
    _getMonitorData(params) {
      getMonitorData(params).then(res => {
        if (res.data) {
          const data = res.data
    ...
    filterOffice(query) {
      ...
      if (query.isAlert === '13') {
        this._getMonitorData({ sensorCat: 4 })
        this.isCo = true
      } else if (query.isAlert === '12' || query.isAlert === '11') {
        this.isCo = false
      }
      this.listQuery = query
      if (this.isCo) {
        this.listQuery = { ...query, sensorCat: 4 }
      }
      this._getOfficeList()
    },
    ...
    switchAlarmList() {
      if (this.showAlarmList === '全部' && this.alarmMsgList.length !== 0) {
        return
      } else {
    ...
    updateData() {
      ...
      this.monitorData = Object.assign({}, this.monitorData, {
        ...
        smknum: monitorData.smknum + latestData.isAlert,
        batnum: monitorData.batnum + latestData.isAlertBattery,
        signum: monitorData.signum + latestData.isAlertSignal
      })
