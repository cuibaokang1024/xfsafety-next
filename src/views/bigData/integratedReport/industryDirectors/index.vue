      ...
      >
        <el-table-column label="单位数" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <span class="pointer" @click="handleRowDataClick(row, 'od')">{{ row.value }}</span>
          </template>
        </el-table-column>
      </BaseTable>
      <BaseTable
...
import { handleIndustryDirectorsData } from '@/api/bigdata'
...
export default {
  ...
  created() {
    this._handleIndustryDirectorsData()
  },
  methods: {
    // 列表
    _handleIndustryDirectorsData() {
      this.listLoading = true
      handleIndustryDirectorsData().then(res => {
        if (res.data) {
