      ...
      >
        <el-table-column label="单位数" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <span class="pointer" @click="handleRowDataClick(row, 'oa')">{{ row.value }}</span>
          </template>
        </el-table-column>
      </BaseTable>
      <BaseTable
...
import { handleAdministrativeDivisionData } from '@/api/bigdata'
...
export default {
  ...
  created() {
    this._handleAdministrativeDivisionData()
  },
  methods: {
    // 列表
    _handleAdministrativeDivisionData() {
      this.listLoading = true
      handleAdministrativeDivisionData().then(res => {
        if (res.data) {
