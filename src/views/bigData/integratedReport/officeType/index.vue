      ...
      >
        <el-table-column label="单位数" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <span class="pointer" @click="handleRowDataClick(row, 'ot')">{{ row.value }}</span>
          </template>
        </el-table-column>
      </BaseTable>
      <BaseTable
...
import { handleOfficeTypeData } from '@/api/bigdata'
...
export default {
  ...
  created() {
    this._handleOfficeTypeData()
  },
  methods: {
    // 列表
    _handleOfficeTypeData() {
      this.listLoading = true
      handleOfficeTypeData().then(res => {
        if (res.data) {
