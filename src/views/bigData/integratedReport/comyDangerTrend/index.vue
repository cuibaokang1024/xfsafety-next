      ...
      >
        <el-table-column label="隐患数量" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <span class="pointer" @click="handleRowDataClick(row, 'com')">{{ row.value }}</span>
          </template>
        </el-table-column>
      </BaseTable>
      <BaseTable
...
import { handleComyDangerTrendData } from '@/api/bigdata'
...
export default {
  ...
  created() {
    this._handleComyDangerTrendData()
  },
  methods: {
    // 列表
    _handleComyDangerTrendData() {
      this.listLoading = true
      handleComyDangerTrendData().then(res => {
        if (res.data) {
