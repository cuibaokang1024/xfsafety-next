  ...
  <div class="report-main">
    ...
    >
      <BaseTable
        ...
        :border="false"
      >
        <el-table-column label="发现隐患" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <span class="pointer" @click="handleRowDataClick(row, 'dd')">{{ row.count }}</span>
          </template>
        </el-table-column>
        <el-table-column label="未整改" align="center" class-name="small-padding fixed-width">
          <template slot-scope="{ row }">
            <span class="pointer" @click="handleRowDataClick(row, 'ddf')">{{ row.fixNo }}</span>
          </template>
        </el-table-column>
      </BaseTable>
      <BaseTable
...
import { handleDangerDirectData } from '@/api/bigdata'
...
export default {
      ...
      const chartData = {
        ...
        serieList: [
          ...
          },
          {
            ...
            data: this.handleInitList(
              this.chartData.uncheckedAxisData,
              this.chartData.ids,
              'ddf'
            ),
            itemStyle: {
  ...
  created() {
    this._handleDangerDirectData()
  },
  methods: {
    // 列表
    _handleDangerDirectData() {
      this.listLoading = true
      handleDangerDirectData().then(res => {
        if (res.data) {
