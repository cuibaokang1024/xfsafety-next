<template>
  <div class="form-box">
    <div name="title" class="dialog-title">消防培训</div>
        ...
        <div v-for="(item, index) in rulesForm" :key="index" class="form-content">
          ...
          <el-col :span="24">
            <el-form-item label="培训内容">
              <el-input v-model="rulesForm[index].content" disabled placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
<script>
import { getFireTrainByOffice } from '@/api/office'
...
export default {
  ...
  created () {
    getFireTrainByOffice({ officeId: this.id }).then(res => {
      console.log('消防培训', res)
    })
  }
}
</script>
